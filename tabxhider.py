...
__module_author__      = "pfn"
__module_updater__     = "Mika Wu"
__module_version__     = "0.2.1.150606"
...
gtkwin = int(hexchat.get_info("gtkwin_ptr"), 16)
...
#        prop = ptr[p].contents
#        hexchat.prnt("%s%2d: %-20s %x %s" % (indent, p,
...
class F:
    ...
    def cb(self, a, data):
        ...
        if gobject.g_type_check_instance_is_a(gobj,
            ...
            #hexchat.prnt("%s0x%016x -- %s" % (indent, a, l.value))

            # third button with a NULL label is the X button, hide it
            if l.value is None:
                self.noneCount = self.noneCount + 1
                if self.noneCount == (3 if version_info[0] <  3 else 4):
                    #hexchat.prnt("Hiding all of the object right above me")
